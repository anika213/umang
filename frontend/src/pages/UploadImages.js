...
const MySwal = withReactContent(Swal);
 ...
 function UploadImages() {
  ...
  async function resetAuction() {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await axios.get(`http://localhost:8000/admin/resetauction?password=${password}`);
  ...
  async function saveInfo(title,artist,description,medium,minBid,styling,theme,dimensions){
    ...
    await axios.put('http://localhost:8000/admin/addpainting', {title,artist,description,medium,minBid,styling,theme,dimensions,image:uploadedImage,paintingnum })
    .then((data) => {
